...
//         VaultReport[] memory reports = oracle.getLatestSharePrices(vaults);
...
//         uint32 srcChainId = 2;
//         VaultReport[] memory reports = new VaultReport[](2);
...
//         VaultReport[] memory storedReports = oracle.getStoredSharePrices(srcChainId, vaultAddresses);
...
//         VaultReport[] memory reports = oracle.getLatestSharePrices(emptyVaults);
//         assertEq(reports.length, 0);
//     }

//     function testGetStoredSharePricesEmpty() public {
//         address[] memory emptyVaults = new address[](0);
//         VaultReport[] memory reports = oracle.getStoredSharePrices(2, emptyVaults);
//         assertEq(reports.length, 0);
//     }
...
//         VaultReport[] memory reports = new VaultReport[](sharePrices.length);
...
//             (uint64 lastUpdate, uint32 chainId, address addr, uint256 price) = oracle.sharePrices(srcChainId,
// vaults[i]);
...
//         VaultReport[] memory updateReports = new VaultReport[](1);
//         updateReports[0] = VaultReport({
//             lastUpdate: uint64(block.timestamp),
//             chainId: CHAIN_ID,
...
//         VaultReport[] memory storedReports = oracle.getStoredSharePrices(CHAIN_ID, vaults);
